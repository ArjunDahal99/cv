...
const UserProfileBoxContainer = ({ data }: any) =>
{
    return (
        <Card className=" w-[400px]  flex flex-col justify-center items-center max-md:w-full ">
            <CardHeader>
                <CardTitle className=" text-center capitalize">{data[0].username}</CardTitle>
            </CardHeader>
            <CardContent className=' flex gap-y-2  flex-col items-center justify-center'>
                <Image className=' rounded-full' src={data[0].profilePicture} width={150} height={150} alt='pp' />
            </CardContent>
            <CardContent className='  opacity-45 flex gap-x-4 items-end justify-center'>
