...
export function AreaChartUsageExampleWithCustomTooltip({ areaData }: any)
{
    ...
    {
        const entryDate = entry.created_At.toLocaleString('en-US', { month: 'short', day: 'numeric' });

        const todayDate = new Date().toLocaleString('en-US', { month: 'short', day: 'numeric' });
        // Check if the entry is from today
        if (entryDate === todayDate)
        {
            const timeKey = entry.created_At.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true });
